...
/**
 * A {@link PTransform} that performs a {@link CoGroupByKeyOnly} on a tuple
 * of tables.  A {@link CoGroupByKeyOnly} groups results from all
 ...
 *
 * It is the same as {@link CoGroupByKey} but instead of using {@link GroupByKey}
 * it uses {@link com.google.cloud.dataflow.sdk.transforms.GroupByKey.GroupByKeyOnly}.
 *
 * <p>Example of performing a {@link CoGroupByKeyOnly} followed by a
 ...
 *                        .and(t2, pt2)
 *                        .apply(CoGroupByKeyOnly.<K>create());
 *
 ...
 * {@code PCollection}s
 */
public class CoGroupByKeyOnly<K> extends
    PTransform<KeyedPCollectionTuple<K>,
               PCollection<KV<K, CoGbkResult>>> {
  /**
   * Returns a {@code CoGroupByKeyOnly<K>} {@code PTransform}.
   *
   * @param <K> the type of the keys in the input and output
   * {@code PCollection}s
   */
  public static <K> CoGroupByKeyOnly<K> create() {
    return new CoGroupByKeyOnly<>();
  }

  private CoGroupByKeyOnly() { }
  ...
  public PCollection<KV<K, CoGbkResult>> apply(
    ...
    PCollection<KV<K, Iterable<RawUnionValue>>> groupedTable =
    	...
    	flattenedTable.apply(new GroupByKey.GroupByKeyOnly<K, RawUnionValue>());	
